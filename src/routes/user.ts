...
export default (app: Router) => {
  ...
  // create User
  route.post('/user/', jsonParser, middlewares.validateSchema(userSchema), (req: Request, res: Response) => {
    logger.info(req);
    users.push(req.body)
  ...
  // update User
  route.put('/user/', jsonParser, middlewares.validateSchema(userSchema), (req: Request, res: Response) => {
    logger.info(req);
  ...
  route.delete('/user/', jsonParser, (req: Request, res: Response) => {
    logger.info(req);
    const user = users.find((item) => item.id === req.body.id);
    if (!user) res.status(404).json({
      status: 'failed',
